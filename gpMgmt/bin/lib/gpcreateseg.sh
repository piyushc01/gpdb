...
CREATE_QES_MIRROR () {
    ...
    if [ $HBA_HOSTNAMES -eq 0 ];then
        local MIRROR_ADDRESSES=($($TRUSTED_SHELL ${GP_HOSTADDRESS} "echo START_CMD_OUTPUT;"${GPHOME}"/libexec/ifaddrs --no-loopback"|$GREP -A1000 -e "^START_CMD_OUTPUT"|$TAIL -n +2))
        local PRIMARY_ADDRESSES=($($TRUSTED_SHELL ${PRIMARY_HOSTADDRESS} "echo START_CMD_OUTPUT;"${GPHOME}"/libexec/ifaddrs --no-loopback"|$GREP -A1000 -e "^START_CMD_OUTPUT"|$TAIL -n +2))
