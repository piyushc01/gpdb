...
CREATE_QES_PRIMARY () {
    ...
    if [ $HBA_HOSTNAMES -eq 0 ]; then
        ...
        SEGMENT_IPV4_LOCAL_ADDRESS_ALL= $( REMOTE_EXECUTE_AND_GET_ARRAY_OUTPUT $GP_HOSTADDRESS "$IPV4_ADDR_LIST_CMD | $GREP inet | $GREP -v \"127.0.0\" | $AWK '{print \$2}' | $CUT -d'/' -f1" )
        ...
        if [ x"" != x"$MIRROR_HOSTADDRESS" ]; then
          ...
          MIRROR_SEGMENT_IPV4_LOCAL_ADDRESS_ALL= $( REMOTE_EXECUTE_AND_GET_ARRAY_OUTPUT $MIRROR_HOSTADDRESS "$IPV4_ADDR_LIST_CMD | $GREP inet | $GREP -v \"127.0.0\" | $AWK '{print \$2}' | $CUT -d'/' -f1" )
          ADD_PG_HBA_ENTRIES "${MIRROR_SEGMENT_IPV4_LOCAL_ADDRESS_ALL[@]}"
        fi
    
        # Add all local IPV6 addresses
        SEGMENT_IPV6_LOCAL_ADDRESS_ALL= $( REMOTE_EXECUTE_AND_GET_ARRAY_OUTPUT $GP_HOSTADDRESS "$IPV6_ADDR_LIST_CMD | $GREP inet6 | $AWK '{print \$2}' | $CUT -d'/' -f1" )
        ADD_PG_HBA_ENTRIES "${SEGMENT_IPV6_LOCAL_ADDRESS_ALL[@]}"
    
        if [ x"" != x"$MIRROR_HOSTADDRESS" ]; then
          # Add all mirror segments local IPV6 addresses
          MIRROR_SEGMENT_IPV6_LOCAL_ADDRESS_ALL= $( REMOTE_EXECUTE_AND_GET_ARRAY_OUTPUT $MIRROR_HOSTADDRESS "$IPV6_ADDR_LIST_CMD | $GREP inet6 | $AWK '{print \$2}' | $CUT -d'/' -f1" ))
...
CREATE_QES_MIRROR () {
    ...
    if [ $HBA_HOSTNAMES -eq 0 ];then
        local MIRROR_ADDRESSES=$( REMOTE_EXECUTE_AND_GET_ARRAY_OUTPUT ${GP_HOSTADDRESS} "'${GPHOME}'/libexec/ifaddrs --no-loopback" )
        local PRIMARY_ADDRESSES=$( REMOTE_EXECUTE_AND_GET_ARRAY_OUTPUT ${PRIMARY_HOSTADDRESS} "'${GPHOME}'/libexec/ifaddrs --no-loopback")
