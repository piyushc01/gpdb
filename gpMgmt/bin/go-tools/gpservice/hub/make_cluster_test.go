...
import (
	"errors"
	"fmt"
	hub2 "github.com/greenplum-db/gpdb/gp/gpservice/hub"
	"maps"
	"os/exec"
	"os/user"
	"reflect"
...
func TestCreateSegments(t *testing.T) {
	...
	hubServer := hub2.New(hubConfig, nil)
...
func TestExecOnDatabase(t *testing.T) {
	...
	t.Run("succesfully executes the query on a given database", func(t *testing.T) {
		...
		err := hub2.ExecOnDatabase(conn, "postgres", "SOME QUERY")
		if err != nil {
			t.Fatalf("unexpected error: %#v", err)
		}
	})
...
func TestCreateGpToolkitExt(t *testing.T) {
	...
	t.Run("succesfully creates the database extensions", func(t *testing.T) {
		hub2.SetExecOnDatabase(func(conn *dbconn.DBConn, dbname, query string) error {
			return nil
		})
		defer hub2.ResetExecOnDatabase()

		conn := &dbconn.DBConn{}
		err := hub2.CreateGpToolkitExt(conn)
		if err != nil {
			t.Fatalf("unexpected error: %#v", err)
		}
	})
...
func TestCreateDatabase(t *testing.T) {
	...
	t.Run("succesfully creates the database", func(t *testing.T) {
		...
		conn := &dbconn.DBConn{}
		err := hub2.CreateDatabase(conn, "testdb")
		if err != nil {
			t.Fatalf("unexpected error: %#v", err)
		}
	})
...
func TestSetGpUserPasswd(t *testing.T) {
	...
	t.Run("succesfully sets the database user password", func(t *testing.T) {
		...
		conn := &dbconn.DBConn{}
		err := hub2.SetGpUserPasswd(conn, "abc")
		if err != nil {
			t.Fatalf("unexpected error: %#v", err)
		}
	})
