...
import (
	"context"
	"fmt"
	idl2 "github.com/greenplum-db/gpdb/gpservice/idl"
	"io"
	"os"
	"path/filepath"
	"regexp"
...
func (s *Server) ValidateHostEnv(ctx context.Context, request *idl2.ValidateHostEnvRequest) (*idl2.ValidateHostEnvReply, error) {
	...
	if utils.System.Getuid() == 0 {
		...
		if err != nil {
			return &idl2.ValidateHostEnvReply{}, utils.LogAndReturnError(fmt.Errorf(
	...
	if gpVersionErr != nil {
		return &idl2.ValidateHostEnvReply{}, utils.LogAndReturnError(fmt.Errorf("Postgres gp-version validation failed:%v", gpVersionErr))
	}

	// Check for each directory if is empty
	nonEmptyDirList, err := GetAllNonEmptyDir(dirList)
	if err != nil {
		return &idl2.ValidateHostEnvReply{}, utils.LogAndReturnError(fmt.Errorf("error checking directory empty:%v", err))
	}

	if len(nonEmptyDirList) > 0 && !forced {
		return &idl2.ValidateHostEnvReply{}, utils.LogAndReturnError(fmt.Errorf("directory not empty:%v", nonEmptyDirList))
	}
	if forced && len(nonEmptyDirList) > 0 {
		...
		for _, dir := range nonEmptyDirList {
			...
			if err != nil {
				return &idl2.ValidateHostEnvReply{}, utils.LogAndReturnError(fmt.Errorf("delete not empty dir:%s, error:%v", dir, err))
			}
		}
	}
	...
	if err != nil {
		return &idl2.ValidateHostEnvReply{}, utils.LogAndReturnError(err)
	}
	...
	return &idl2.ValidateHostEnvReply{Messages: warnings}, nil
}
...
func CheckOpenFilesLimit() []*idl2.LogMessage {
	var warnings []*idl2.LogMessage
	ulimitVal, err := utils.ExecuteAndGetUlimit()
	if err != nil {
		warnMsg := fmt.Sprintf("error getting open files limit:%s", err.Error())
		warnings = append(warnings, &idl2.LogMessage{Message: warnMsg, Level: idl2.LogLevel_WARNING})
		gplog.Warn(warnMsg)
		return warnings
	}
...
*/
func CheckHostAddressInHostsFile(hostAddressList []string) []*idl2.LogMessage {
	var warnings []*idl2.LogMessage
...
func ValidateLocaleSettingsFn(locale *idl2.Locale) error {
