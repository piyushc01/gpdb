...
import (
	"errors"
	"fmt"
	cli "github.com/greenplum-db/gpdb/gpservice/gpservice-cli"
...
func TestValidateMultiHomeConfig(t *testing.T) {
	...
	t.Run("returns error when the hosts in the host list has different number of interfaces", func(t *testing.T) {
		...
		_, err := cli2.ValidateMultiHomeConfig(config, addressMap)
	...
	t.Run("returns error when the directories per host are not in multiple of number of interfaces", func(t *testing.T) {
		...
		config := cli2.InitConfig{PrimaryDataDirectories: []string{"/tmp", "/tmp", "/tmp"}}
		addressMap := map[string][]string{
			"sdw1": {"sdw1-1", "sdw1-2"},
			"sdw2": {"sdw2-1", "sdw2-2"},
			"sdw3": {"sdw3-1", "sdw3-2"},
	...
	t.Run("returns error when spread mirroring and number of unique hosts is equal", func(t *testing.T) {
		...
		config := cli2.InitConfig{PrimaryDataDirectories: []string{"/tmp", "/tmp", "/tmp", "/tmp"},
...
func TestValidateExpansionConfigAndSetDefault(t *testing.T) {
	...
	t.Run("returns error when primary data directories not provided", func(t *testing.T) {
		...
		err := cli2.ValidateExpansionConfigAndSetDefault(config, cliHandle)
		if err == nil || !strings.Contains(err.Error(), testStr) {
	...
	t.Run("returns error when hostlist not provided", func(t *testing.T) {
		...
		config := &cli2.InitConfig{PrimaryDataDirectories: []string{"/test"}}
	...
	t.Run("returns error when primary data directory contains empty string", func(t *testing.T) {
		...
		config := &cli2.InitConfig{PrimaryDataDirectories: []string{""}}
	...
	t.Run("returns error when primary data directory contains empty and non-empty string", func(t *testing.T) {
		...
		config := &cli2.InitConfig{PrimaryDataDirectories: []string{"/test1", "", "/test2"}}
	...
	t.Run("returns error when hostlist contains empty string", func(t *testing.T) {
		...
		config := &cli2.InitConfig{PrimaryDataDirectories: []string{"/test"}, HostList: []string{""}}
	...
	t.Run("returns error when hostlist contains empty and non-empty string", func(t *testing.T) {
		...
		config := &cli2.InitConfig{PrimaryDataDirectories: []string{"/test"}, HostList: []string{"/tmp/1", "", "/tmp/2"}}
	...
	t.Run("returns error when primary base port value is same as coordinator port", func(t *testing.T) {
		...
		config := &cli2.InitConfig{PrimaryDataDirectories: []string{"/test"}, HostList: []string{"swd1"}, Coordinator: cli2.Segment{Port: basePort}, PrimaryBasePort: basePort}
	...
	t.Run("sets ContainsMirror to false and default value to primary base port", func(t *testing.T) {
		...
		cliHandle.Set("primary-data-directories", []string{"/test"})
		cliHandle.Set("hostlist", []string{"swd1"})
		config := &cli2.InitConfig{PrimaryDataDirectories: []string{"/test"}, HostList: []string{"swd1"}, Coordinator: cli2.Segment{Port: basePort}}
		...
		if cli2.ContainsMirror {
			t.Fatalf("PrimaryBasePort got:%v, expected:false", cli2.ContainsMirror)
		}
	})
	...
	t.Run("returns error when segment array is defined and expansion parameters also present", func(t *testing.T) {
		...
		cliHandle := viper.New()
		basePort := 9000
		segArray := []cli2.SegmentPair{{Primary: &cli2.Segment{Hostname: "sdw1"}}}
	...
	t.Run("sets default mirror port value properly", func(t *testing.T) {
		...
		config := &cli2.InitConfig{PrimaryDataDirectories: []string{"/test"}, HostList: []string{"swd1"}, Coordinator: cli2.Segment{Port: basePort}, MirrorDataDirectories: []string{"/test1"}, MirroringType: constants.GroupMirroring}
	...
	t.Run("sets default mirroring type value properly", func(t *testing.T) {
		...
		config := &cli2.InitConfig{PrimaryDataDirectories: []string{"/test"}, HostList: []string{"swd1"}, Coordinator: cli2.Segment{Port: basePort}, MirrorDataDirectories: []string{"/test1"}, MirrorBasePort: 10000}
		cliHandle.Set("mirror-base-port", basePort)
		cliHandle.Set("primary-data-directories", []string{"/test"})
		cliHandle.Set("hostlist", []string{"swd1"})
	...
	t.Run("returns error when number of hosts are less in spread mirroring", func(t *testing.T) {
		...
		cliHandle := viper.New()
		basePort := 9000
		config := &cli2.InitConfig{PrimaryDataDirectories: []string{"/test1", "/test2", "/test3"}, HostList: []string{"swd1", "sdw2"}, Coordinator: cli2.Segment{Port: basePort}, MirrorDataDirectories: []string{"/mirror1", "/mirror2", "/mirror3"}, MirroringType: constants.SpreadMirroring}
	...
	t.Run("returns error when number of hosts are equal in spread mirroring", func(t *testing.T) {
		...
		cliHandle := viper.New()
		basePort := 9000
		config := &cli2.InitConfig{PrimaryDataDirectories: []string{"/test1", "/test2", "/test3"}, HostList: []string{"swd1", "sdw2", "sdw3"}, Coordinator: cli2.Segment{Port: basePort}, MirrorDataDirectories: []string{"/mirror1", "/mirror2", "/mirror3"}, MirroringType: constants.SpreadMirroring}
		cliHandle.Set("mirroring-type", constants.SpreadMirroring)
...
func TestIsMultiHome(t *testing.T) {
	...
	t.Run("returns error when error getting all hostnames", func(t *testing.T) {
		testStr := "test Error"
		hostList := []string{"cdw"}
		cdw := mock_idl.NewMockHubClient(cli.ctrl)
		...
		oldHubClient := cli2.HubClient
		cli2.HubClient = cdw
		defer func() { cli2.HubClient = oldHubClient }()

		_, _, _, err := cli2.IsMultiHome(hostList)
		if err == nil || !strings.Contains(err.Error(), testStr) {
			t.Fatalf("Got:%v, Expected:%s", err, testStr)
		}
	})
	...
	t.Run("returns false and other details properly when not a multi-home", func(t *testing.T) {
		...
		isMultiHome, nameAddressMap, addressNameMap, err := cli2.IsMultiHome(hostList)
		if err != nil {
			t.Fatalf("Got:%v, Expected:no error", err)
		}
...
func CheckIfPortConflict(segPairList *[]cli2.SegmentPair) error {
...
func TestExpandNonMultiHomePrimaryList(t *testing.T) {
	...
	t.Run("should expand primary array for a non multi home setup", func(t *testing.T) {
		var segPairList []cli2.SegmentPair
		...
		expectedSegPairList := []cli2.SegmentPair{
			{Primary: &cli2.Segment{Hostname: "sdw1", Address: "sdw1", Port: 9000, DataDirectory: "/test/gpseg0"}},
			{Primary: &cli2.Segment{Hostname: "sdw1", Address: "sdw1", Port: 9001, DataDirectory: "/test/gpseg1"}},
			{Primary: &cli2.Segment{Hostname: "sdw2", Address: "sdw2", Port: 9000, DataDirectory: "/test/gpseg2"}},
			{Primary: &cli2.Segment{Hostname: "sdw2", Address: "sdw2", Port: 9001, DataDirectory: "/test/gpseg3"}},
		}

		segPairList = *cli2.ExpandNonMultiHomePrimaryList(&segPairList, primaryBasePort, primaryBaseDataDirectories, hostList, addressNameMap)
	...
	t.Run("should expand primary array for a non multi home setup with different mount-points", func(t *testing.T) {
		...
		addressNameMap := map[string]string{"sdw1": "sdw1", "sdw2": "sdw2"}
		expectedSegPairList := []cli2.SegmentPair{
			{Primary: &cli2.Segment{Hostname: "sdw1", Address: "sdw1", Port: 9000, DataDirectory: "/test1/gpseg0"}},
			{Primary: &cli2.Segment{Hostname: "sdw1", Address: "sdw1", Port: 9001, DataDirectory: "/test2/gpseg1"}},
			{Primary: &cli2.Segment{Hostname: "sdw2", Address: "sdw2", Port: 9000, DataDirectory: "/test1/gpseg2"}},
			{Primary: &cli2.Segment{Hostname: "sdw2", Address: "sdw2", Port: 9001, DataDirectory: "/test2/gpseg3"}},
		}
...
func TestExpandMultiHomePrimaryArray(t *testing.T) {
	...
	t.Run("should expand primary array for a multi home setup", func(t *testing.T) {
		...
		expectedSegPairList := []cli2.SegmentPair{
			{Primary: &cli2.Segment{Hostname: "sdw1", Address: "sdw1-1", Port: 9000, DataDirectory: "/test/gpseg0"}},
			{Primary: &cli2.Segment{Hostname: "sdw1", Address: "sdw1-2", Port: 9001, DataDirectory: "/test/gpseg1"}},
			{Primary: &cli2.Segment{Hostname: "sdw2", Address: "sdw2-1", Port: 9000, DataDirectory: "/test/gpseg2"}},
			{Primary: &cli2.Segment{Hostname: "sdw2", Address: "sdw2-2", Port: 9001, DataDirectory: "/test/gpseg3"}},
		}

		segPairList = *cli2.ExpandMultiHomePrimaryArray(&segPairList, primaryBasePort, primaryBaseDataDirectories, uniqHostnameList, nameAddressMap)
		if !reflect.DeepEqual(segPairList, expectedSegPairList) {
			t.Fatalf("Got:%+v Expected %+v", segPairList, expectedSegPairList)
		}
		err := CheckIfPortConflict(&segPairList)
	...
	t.Run("should expand primary array for a multi home setup multi mount point", func(t *testing.T) {
		...
		expectedSegPairList := []cli2.SegmentPair{
			{Primary: &cli2.Segment{Hostname: "sdw1", Address: "sdw1-1", Port: 9000, DataDirectory: "/test1/gpseg0"}},
			{Primary: &cli2.Segment{Hostname: "sdw1", Address: "sdw1-2", Port: 9001, DataDirectory: "/test2/gpseg1"}},
			{Primary: &cli2.Segment{Hostname: "sdw2", Address: "sdw2-1", Port: 9000, DataDirectory: "/test1/gpseg2"}},
			{Primary: &cli2.Segment{Hostname: "sdw2", Address: "sdw2-2", Port: 9001, DataDirectory: "/test2/gpseg3"}},
		}
...
func TestExpandNonMultiHomeGroupMirrorList(t *testing.T) {
	...
	t.Run("should expand mirror array for a non multi home setup", func(t *testing.T) {
		...
		addressNameMap := map[string]string{"sdw1": "sdw1", "sdw2": "sdw2"}
		expectedSegPairList := []cli2.SegmentPair{
			{Mirror: &cli2.Segment{Hostname: "sdw2", Address: "sdw2", Port: 9000, DataDirectory: "/test/gpseg0"}},
			{Mirror: &cli2.Segment{Hostname: "sdw2", Address: "sdw2", Port: 9001, DataDirectory: "/test/gpseg1"}},
			{Mirror: &cli2.Segment{Hostname: "sdw1", Address: "sdw1", Port: 9000, DataDirectory: "/test/gpseg2"}},
			{Mirror: &cli2.Segment{Hostname: "sdw1", Address: "sdw1", Port: 9001, DataDirectory: "/test/gpseg3"}},
		}
		segPairList = make([]cli2.SegmentPair, 4)

		segPairList = *cli2.ExpandNonMultiHomeGroupMirrorList(&segPairList, mirrorBasePort, mirrorBaseDataDirectories, hostList, addressNameMap)
		if !reflect.DeepEqual(segPairList, expectedSegPairList) {
			t.Fatalf("Got:%+v Expected %+v", segPairList, expectedSegPairList)
		}
		err := CheckIfPortConflict(&segPairList)
	...
	t.Run("should expand mirror array for a non multi home setup with separate mount", func(t *testing.T) {
		...
		addressNameMap := map[string]string{"sdw1": "sdw1", "sdw2": "sdw2"}
		expectedSegPairList := []cli2.SegmentPair{
			{Mirror: &cli2.Segment{Hostname: "sdw2", Address: "sdw2", Port: 9000, DataDirectory: "/test1/gpseg0"}},
			{Mirror: &cli2.Segment{Hostname: "sdw2", Address: "sdw2", Port: 9001, DataDirectory: "/test2/gpseg1"}},
			{Mirror: &cli2.Segment{Hostname: "sdw1", Address: "sdw1", Port: 9000, DataDirectory: "/test1/gpseg2"}},
			{Mirror: &cli2.Segment{Hostname: "sdw1", Address: "sdw1", Port: 9001, DataDirectory: "/test2/gpseg3"}},
		}
		segPairList = make([]cli2.SegmentPair, 4)
...
func TestExpandNonMultiHomeSpreadMirrorList(t *testing.T) {
	...
	t.Run("should expand mirror array for a non multi home setup - spread mirroring", func(t *testing.T) {
		...
		expectedSegPairList := []cli2.SegmentPair{
			{Mirror: &cli2.Segment{Hostname: "sdw2", Address: "sdw2", Port: 9000, DataDirectory: "/test/gpseg0"}},
			{Mirror: &cli2.Segment{Hostname: "sdw3", Address: "sdw3", Port: 9001, DataDirectory: "/test/gpseg1"}},
			{Mirror: &cli2.Segment{Hostname: "sdw3", Address: "sdw3", Port: 9000, DataDirectory: "/test/gpseg2"}},
			{Mirror: &cli2.Segment{Hostname: "sdw1", Address: "sdw1", Port: 9001, DataDirectory: "/test/gpseg3"}},
			{Mirror: &cli2.Segment{Hostname: "sdw1", Address: "sdw1", Port: 9000, DataDirectory: "/test/gpseg4"}},
			{Mirror: &cli2.Segment{Hostname: "sdw2", Address: "sdw2", Port: 9001, DataDirectory: "/test/gpseg5"}},
		}
		segPairList = make([]cli2.SegmentPair, 6)

		segPairList = *cli2.ExpandNonMultiHomeSpreadMirroring(&segPairList, mirrorBasePort, mirrorBaseDataDirectories, hostList, addressNameMap)
		if !reflect.DeepEqual(segPairList, expectedSegPairList) {
			t.Fatalf("Got:%+v Expected %+v", segPairList, expectedSegPairList)
		}
		err := CheckIfPortConflict(&segPairList)
	...
	t.Run("should expand mirror array for a non multi home setup with separate mount - spread mirroring", func(t *testing.T) {
		...
		expectedSegPairList := []cli2.SegmentPair{
			{Mirror: &cli2.Segment{Hostname: "sdw2", Address: "sdw2", Port: 9000, DataDirectory: "/test1/gpseg0"}},
			{Mirror: &cli2.Segment{Hostname: "sdw3", Address: "sdw3", Port: 9001, DataDirectory: "/test2/gpseg1"}},
			{Mirror: &cli2.Segment{Hostname: "sdw3", Address: "sdw3", Port: 9000, DataDirectory: "/test1/gpseg2"}},
			{Mirror: &cli2.Segment{Hostname: "sdw1", Address: "sdw1", Port: 9001, DataDirectory: "/test2/gpseg3"}},
			{Mirror: &cli2.Segment{Hostname: "sdw1", Address: "sdw1", Port: 9000, DataDirectory: "/test1/gpseg4"}},
			{Mirror: &cli2.Segment{Hostname: "sdw2", Address: "sdw2", Port: 9001, DataDirectory: "/test2/gpseg5"}},
		}
		segPairList = make([]cli2.SegmentPair, 6)
...
func TestExpandMultiHomeGroupMirrorList(t *testing.T) {
	...
	t.Run("should expand mirror array for a multi home setup - group mirroring", func(t *testing.T) {
		...
		expectedSegPairList := []cli2.SegmentPair{
			{Mirror: &cli2.Segment{Hostname: "sdw2", Address: "sdw2-1", Port: 9000, DataDirectory: "/test/gpseg0"}},
			{Mirror: &cli2.Segment{Hostname: "sdw2", Address: "sdw2-2", Port: 9001, DataDirectory: "/test/gpseg1"}},
			{Mirror: &cli2.Segment{Hostname: "sdw1", Address: "sdw1-1", Port: 9000, DataDirectory: "/test/gpseg2"}},
			{Mirror: &cli2.Segment{Hostname: "sdw1", Address: "sdw1-2", Port: 9001, DataDirectory: "/test/gpseg3"}},
		}

		segPairList = *cli2.ExpandMultiHomeGroupMirrorList(&segPairList, mirrorBasePort, mirrorBaseDataDirectories, uniqHostnameList, nameAddressMap)
		if !reflect.DeepEqual(segPairList, expectedSegPairList) {
			t.Fatalf("Got:%+v Expected %+v", segPairList, expectedSegPairList)
		}
		err := CheckIfPortConflict(&segPairList)
	...
	t.Run("should expand mirror array for a multi home setup - group mirroring with separate mount point", func(t *testing.T) {
		...
		expectedSegPairList := []cli2.SegmentPair{
			{Mirror: &cli2.Segment{Hostname: "sdw2", Address: "sdw2-1", Port: 9000, DataDirectory: "/test1/gpseg0"}},
			{Mirror: &cli2.Segment{Hostname: "sdw2", Address: "sdw2-2", Port: 9001, DataDirectory: "/test2/gpseg1"}},
			{Mirror: &cli2.Segment{Hostname: "sdw1", Address: "sdw1-1", Port: 9000, DataDirectory: "/test1/gpseg2"}},
			{Mirror: &cli2.Segment{Hostname: "sdw1", Address: "sdw1-2", Port: 9001, DataDirectory: "/test2/gpseg3"}},
		}
...
func TestExpandMultiHomeSpreadMirrorList(t *testing.T) {
	...
	t.Run("should expand mirror array for a multi home setup - spread mirroring", func(t *testing.T) {
		...
		expectedSegPairList := []cli2.SegmentPair{
			{Mirror: &cli2.Segment{Hostname: "sdw2", Address: "sdw2-1", Port: 9000, DataDirectory: "/test/gpseg0"}},
			{Mirror: &cli2.Segment{Hostname: "sdw3", Address: "sdw3-2", Port: 9001, DataDirectory: "/test/gpseg1"}},
			{Mirror: &cli2.Segment{Hostname: "sdw3", Address: "sdw3-2", Port: 9000, DataDirectory: "/test/gpseg2"}},
			{Mirror: &cli2.Segment{Hostname: "sdw1", Address: "sdw1-1", Port: 9001, DataDirectory: "/test/gpseg3"}},
			{Mirror: &cli2.Segment{Hostname: "sdw1", Address: "sdw1-1", Port: 9000, DataDirectory: "/test/gpseg4"}},
			{Mirror: &cli2.Segment{Hostname: "sdw2", Address: "sdw2-2", Port: 9001, DataDirectory: "/test/gpseg5"}},
		}

		segPairList = *cli2.ExpandMultiHomeSpreadMirrorList(&segPairList, mirrorBasePort, mirrorBaseDataDirectories, uniqHostnameList, nameAddressMap)
		if !reflect.DeepEqual(segPairList, expectedSegPairList) {
			t.Fatalf("Got:%+v Expected %+v", segPairList, expectedSegPairList)
		}
		err := CheckIfPortConflict(&segPairList)
	...
	t.Run("should expand mirror array for a multi home setup - group mirroring with separate mount point", func(t *testing.T) {
		...
		expectedSegPairList := []cli2.SegmentPair{
			{Mirror: &cli2.Segment{Hostname: "sdw2", Address: "sdw2-1", Port: 9000, DataDirectory: "/test1/gpseg0"}},
			{Mirror: &cli2.Segment{Hostname: "sdw3", Address: "sdw3-2", Port: 9001, DataDirectory: "/test2/gpseg1"}},
			{Mirror: &cli2.Segment{Hostname: "sdw3", Address: "sdw3-2", Port: 9000, DataDirectory: "/test1/gpseg2"}},
			{Mirror: &cli2.Segment{Hostname: "sdw1", Address: "sdw1-1", Port: 9001, DataDirectory: "/test2/gpseg3"}},
			{Mirror: &cli2.Segment{Hostname: "sdw1", Address: "sdw1-1", Port: 9000, DataDirectory: "/test1/gpseg4"}},
			{Mirror: &cli2.Segment{Hostname: "sdw2", Address: "sdw2-2", Port: 9001, DataDirectory: "/test2/gpseg5"}},
		}
...
func TestExpandSegPairArray(t *testing.T) {
	...
	t.Run("returns list of expansion for the given config when contains only primaries non multi-home", func(t *testing.T) {
		...
		cli2.ContainsMirror = false

		segPairList := cli2.ExpandSegPairArray(config, false, nameAddressMap, addressNameMap)
	...
	t.Run("returns list of expansion for the given config when contains only primaries non multi-home and given address list", func(t *testing.T) {
		...
		expectedSegPairList := []cli2.SegmentPair{
			{Primary: &cli2.Segment{Hostname: "sdw1", Address: "sdw1-1", Port: 9000, DataDirectory: "/test/gpseg0"}},
			{Primary: &cli2.Segment{Hostname: "sdw1", Address: "sdw1-1", Port: 9001, DataDirectory: "/test/gpseg1"}},
			{Primary: &cli2.Segment{Hostname: "sdw2", Address: "sdw2-1", Port: 9000, DataDirectory: "/test/gpseg2"}},
			{Primary: &cli2.Segment{Hostname: "sdw2", Address: "sdw2-1", Port: 9001, DataDirectory: "/test/gpseg3"}},
		}
		cli2.ContainsMirror = false
	...
	t.Run("returns list of expansion for the given config when contains only primaries on a multi-home", func(t *testing.T) {
		...
		segPairList := cli2.ExpandSegPairArray(config, true, nameAddressMap, addressNameMap)
		if len(segPairList) != 4 {
			t.Fatalf("Got segPairList length %d, expected length: 4", len(segPairList))
		}
		if !reflect.DeepEqual(segPairList, expectedSegPairList) {
	...
	t.Run("returns list of expansion for the given config contain primary and group mirrors non multi-home", func(t *testing.T) {
		...
		expectedSegPairList := []cli2.SegmentPair{
			{Primary: &cli2.Segment{Hostname: "sdw1", Address: "sdw1", Port: 9000, DataDirectory: "/test/gpseg0"},
				Mirror: &cli2.Segment{Hostname: "sdw2", Address: "sdw2", Port: 10000, DataDirectory: "/mirror/gpseg0"}},

			{Primary: &cli2.Segment{Hostname: "sdw1", Address: "sdw1", Port: 9001, DataDirectory: "/test/gpseg1"},
				Mirror: &cli2.Segment{Hostname: "sdw2", Address: "sdw2", Port: 10001, DataDirectory: "/mirror/gpseg1"}},
			{Primary: &cli2.Segment{Hostname: "sdw2", Address: "sdw2", Port: 9000, DataDirectory: "/test/gpseg2"},
				Mirror: &cli2.Segment{Hostname: "sdw1", Address: "sdw1", Port: 10000, DataDirectory: "/mirror/gpseg2"}},
			{Primary: &cli2.Segment{Hostname: "sdw2", Address: "sdw2", Port: 9001, DataDirectory: "/test/gpseg3"},
				Mirror: &cli2.Segment{Hostname: "sdw1", Address: "sdw1", Port: 10001, DataDirectory: "/mirror/gpseg3"}},
		}
		cli2.ContainsMirror = true
	...
	t.Run("returns list of expansion for the given config contain primary and spread mirrors non multi-home", func(t *testing.T) {
		...
		expectedSegPairList := []cli2.SegmentPair{
			{Primary: &cli2.Segment{Hostname: "sdw1", Address: "sdw1", Port: 9000, DataDirectory: "/test/gpseg0"},
				Mirror: &cli2.Segment{Hostname: "sdw2", Address: "sdw2", Port: 10000, DataDirectory: "/mirror/gpseg0"}},
			{Primary: &cli2.Segment{Hostname: "sdw1", Address: "sdw1", Port: 9001, DataDirectory: "/test/gpseg1"},
				Mirror: &cli2.Segment{Hostname: "sdw3", Address: "sdw3", Port: 10001, DataDirectory: "/mirror/gpseg1"}},
			{Primary: &cli2.Segment{Hostname: "sdw2", Address: "sdw2", Port: 9000, DataDirectory: "/test/gpseg2"},
				Mirror: &cli2.Segment{Hostname: "sdw3", Address: "sdw3", Port: 10000, DataDirectory: "/mirror/gpseg2"}},
			{Primary: &cli2.Segment{Hostname: "sdw2", Address: "sdw2", Port: 9001, DataDirectory: "/test/gpseg3"},
				Mirror: &cli2.Segment{Hostname: "sdw1", Address: "sdw1", Port: 10001, DataDirectory: "/mirror/gpseg3"}},
			{Primary: &cli2.Segment{Hostname: "sdw3", Address: "sdw3", Port: 9000, DataDirectory: "/test/gpseg4"},
				Mirror: &cli2.Segment{Hostname: "sdw1", Address: "sdw1", Port: 10000, DataDirectory: "/mirror/gpseg4"}},
			{Primary: &cli2.Segment{Hostname: "sdw3", Address: "sdw3", Port: 9001, DataDirectory: "/test/gpseg5"},
				Mirror: &cli2.Segment{Hostname: "sdw2", Address: "sdw2", Port: 10001, DataDirectory: "/mirror/gpseg5"}},
		}
		cli2.ContainsMirror = true
	...
	t.Run("returns list of expansion for the given config contain primary and group mirrors non multi-home separate mount points", func(t *testing.T) {
		...
		expectedSegPairList := []cli2.SegmentPair{
			{Primary: &cli2.Segment{Hostname: "sdw1", Address: "sdw1", Port: 9000, DataDirectory: "/test1/gpseg0"},
				Mirror: &cli2.Segment{Hostname: "sdw2", Address: "sdw2", Port: 10000, DataDirectory: "/mirror1/gpseg0"}},
			{Primary: &cli2.Segment{Hostname: "sdw1", Address: "sdw1", Port: 9001, DataDirectory: "/test2/gpseg1"},
				Mirror: &cli2.Segment{Hostname: "sdw2", Address: "sdw2", Port: 10001, DataDirectory: "/mirror2/gpseg1"}},
			{Primary: &cli2.Segment{Hostname: "sdw2", Address: "sdw2", Port: 9000, DataDirectory: "/test1/gpseg2"},
				Mirror: &cli2.Segment{Hostname: "sdw1", Address: "sdw1", Port: 10000, DataDirectory: "/mirror1/gpseg2"}},
			{Primary: &cli2.Segment{Hostname: "sdw2", Address: "sdw2", Port: 9001, DataDirectory: "/test2/gpseg3"},
				Mirror: &cli2.Segment{Hostname: "sdw1", Address: "sdw1", Port: 10001, DataDirectory: "/mirror2/gpseg3"}},
		}
		cli2.ContainsMirror = true
	...
	t.Run("returns list of expansion for the given config contain primary and group mirrors non multi-home when given address", func(t *testing.T) {
		...
		expectedSegPairList := []cli2.SegmentPair{
			{Primary: &cli2.Segment{Hostname: "sdw1", Address: "sdw1-1", Port: 9000, DataDirectory: "/test/gpseg0"},
				Mirror: &cli2.Segment{Hostname: "sdw2", Address: "sdw2-1", Port: 10000, DataDirectory: "/mirror/gpseg0"}},

			{Primary: &cli2.Segment{Hostname: "sdw1", Address: "sdw1-1", Port: 9001, DataDirectory: "/test/gpseg1"},
				Mirror: &cli2.Segment{Hostname: "sdw2", Address: "sdw2-1", Port: 10001, DataDirectory: "/mirror/gpseg1"}},
			{Primary: &cli2.Segment{Hostname: "sdw2", Address: "sdw2-1", Port: 9000, DataDirectory: "/test/gpseg2"},
				Mirror: &cli2.Segment{Hostname: "sdw1", Address: "sdw1-1", Port: 10000, DataDirectory: "/mirror/gpseg2"}},
			{Primary: &cli2.Segment{Hostname: "sdw2", Address: "sdw2-1", Port: 9001, DataDirectory: "/test/gpseg3"},
				Mirror: &cli2.Segment{Hostname: "sdw1", Address: "sdw1-1", Port: 10001, DataDirectory: "/mirror/gpseg3"}},
		}
		cli2.ContainsMirror = true
	...
	t.Run("returns list of expansion for the given config contain primary and spread mirrors non multi-home when given address", func(t *testing.T) {
		...
		expectedSegPairList := []cli2.SegmentPair{
			{Primary: &cli2.Segment{Hostname: "sdw1", Address: "sdw1-1", Port: 9000, DataDirectory: "/test/gpseg0"},
				Mirror: &cli2.Segment{Hostname: "sdw2", Address: "sdw2-1", Port: 10000, DataDirectory: "/mirror/gpseg0"}},
			{Primary: &cli2.Segment{Hostname: "sdw1", Address: "sdw1-1", Port: 9001, DataDirectory: "/test/gpseg1"},
				Mirror: &cli2.Segment{Hostname: "sdw3", Address: "sdw3-1", Port: 10001, DataDirectory: "/mirror/gpseg1"}},
			{Primary: &cli2.Segment{Hostname: "sdw2", Address: "sdw2-1", Port: 9000, DataDirectory: "/test/gpseg2"},
				Mirror: &cli2.Segment{Hostname: "sdw3", Address: "sdw3-1", Port: 10000, DataDirectory: "/mirror/gpseg2"}},
			{Primary: &cli2.Segment{Hostname: "sdw2", Address: "sdw2-1", Port: 9001, DataDirectory: "/test/gpseg3"},
				Mirror: &cli2.Segment{Hostname: "sdw1", Address: "sdw1-1", Port: 10001, DataDirectory: "/mirror/gpseg3"}},
			{Primary: &cli2.Segment{Hostname: "sdw3", Address: "sdw3-1", Port: 9000, DataDirectory: "/test/gpseg4"},
				Mirror: &cli2.Segment{Hostname: "sdw1", Address: "sdw1-1", Port: 10000, DataDirectory: "/mirror/gpseg4"}},
			{Primary: &cli2.Segment{Hostname: "sdw3", Address: "sdw3-1", Port: 9001, DataDirectory: "/test/gpseg5"},
				Mirror: &cli2.Segment{Hostname: "sdw2", Address: "sdw2-1", Port: 10001, DataDirectory: "/mirror/gpseg5"}},
		}
		cli2.ContainsMirror = true
	...
	t.Run("returns list of expansion for the given config when contains primaries and group mirror on a multi-home", func(t *testing.T) {
		...
		expectedSegPairList := []cli2.SegmentPair{
			{Primary: &cli2.Segment{Hostname: "sdw1", Address: "sdw1-1", Port: 9000, DataDirectory: "/test/gpseg0"},
				Mirror: &cli2.Segment{Hostname: "sdw2", Address: "sdw2-1", Port: 10000, DataDirectory: "/mirror/gpseg0"}},
			{Primary: &cli2.Segment{Hostname: "sdw1", Address: "sdw1-2", Port: 9001, DataDirectory: "/test/gpseg1"},
				Mirror: &cli2.Segment{Hostname: "sdw2", Address: "sdw2-2", Port: 10001, DataDirectory: "/mirror/gpseg1"}},
			{Primary: &cli2.Segment{Hostname: "sdw2", Address: "sdw2-1", Port: 9000, DataDirectory: "/test/gpseg2"},
				Mirror: &cli2.Segment{Hostname: "sdw1", Address: "sdw1-1", Port: 10000, DataDirectory: "/mirror/gpseg2"}},
			{Primary: &cli2.Segment{Hostname: "sdw2", Address: "sdw2-2", Port: 9001, DataDirectory: "/test/gpseg3"},
				Mirror: &cli2.Segment{Hostname: "sdw1", Address: "sdw1-2", Port: 10001, DataDirectory: "/mirror/gpseg3"}},
		}
		cli2.ContainsMirror = true
	...
	t.Run("returns list of expansion for the given config when contains primaries and spread mirror on a multi-home", func(t *testing.T) {
		...
		expectedSegPairList := []cli2.SegmentPair{
			{Primary: &cli2.Segment{Hostname: "sdw1", Address: "sdw1-1", Port: 9000, DataDirectory: "/test/gpseg0"},
				Mirror: &cli2.Segment{Hostname: "sdw2", Address: "sdw2-1", Port: 10000, DataDirectory: "/mirror/gpseg0"}},
			{Primary: &cli2.Segment{Hostname: "sdw1", Address: "sdw1-2", Port: 9001, DataDirectory: "/test/gpseg1"},
				Mirror: &cli2.Segment{Hostname: "sdw3", Address: "sdw3-2", Port: 10001, DataDirectory: "/mirror/gpseg1"}},
			{Primary: &cli2.Segment{Hostname: "sdw2", Address: "sdw2-1", Port: 9000, DataDirectory: "/test/gpseg2"},
				Mirror: &cli2.Segment{Hostname: "sdw3", Address: "sdw3-2", Port: 10000, DataDirectory: "/mirror/gpseg2"}},
			{Primary: &cli2.Segment{Hostname: "sdw2", Address: "sdw2-2", Port: 9001, DataDirectory: "/test/gpseg3"},
				Mirror: &cli2.Segment{Hostname: "sdw1", Address: "sdw1-1", Port: 10001, DataDirectory: "/mirror/gpseg3"}},
			{Primary: &cli2.Segment{Hostname: "sdw3", Address: "sdw3-1", Port: 9000, DataDirectory: "/test/gpseg4"},
				Mirror: &cli2.Segment{Hostname: "sdw1", Address: "sdw1-1", Port: 10000, DataDirectory: "/mirror/gpseg4"}},
			{Primary: &cli2.Segment{Hostname: "sdw3", Address: "sdw3-2", Port: 9001, DataDirectory: "/test/gpseg5"},
				Mirror: &cli2.Segment{Hostname: "sdw2", Address: "sdw2-2", Port: 10001, DataDirectory: "/mirror/gpseg5"}},
		}
		cli2.ContainsMirror = true
...
func TestRunInitClusterCmd(t *testing.T) {
	...
	t.Run("returns error when length of args less than 1", func(t *testing.T) {
		...
		cmd := cobra.Command{}
		err := cli2.RunInitClusterCmd(&cmd, nil)
		if err == nil || !strings.Contains(err.Error(), testStr) {
			t.Fatalf("got:%v, expected:%s", err, testStr)
		}
	})
	t.Run("returns error when length of args greater than 1", func(t *testing.T) {
		...
		err := cli2.RunInitClusterCmd(&cmd, args)
		if err == nil || !strings.Contains(err.Error(), testStr) {
			t.Fatalf("got:%v, expected:%s", err, testStr)
		}
	})
	t.Run("returns error cluster creation fails", func(t *testing.T) {
		...
		cmd := cobra.Command{}
		args := []string{"/tmp/1"}
		cli2.InitClusterService = func(inputConfigFile string, force, verbose bool) error {
			return fmt.Errorf(testStr)
		}
		defer cli.resetCLIVars()
		err := cli2.RunInitClusterCmd(&cmd, args)
		if err == nil || !strings.Contains(err.Error(), testStr) {
			t.Fatalf("got:%v, expected:%s", err, testStr)
		}
...
func TestInitClusterService(t *testing.T) {
	...
	t.Run("fails if input config file does not exist", func(t *testing.T) {
		defer cli.resetCLIVars()
		err := cli2.InitClusterService("/tmp/invalid_file", false, false)
		if err == nil {
	...
	t.Run("fails if LoadInputConfigToIdl returns error", func(t *testing.T) {
		...
		cli2.LoadInputConfigToIdl = func(inputConfigFile string, cliHandler *viper.Viper, force bool, verbose bool) (*idl.MakeClusterRequest, error) {
			return nil, fmt.Errorf(testStr)
		}
		ConnectToHub = func(conf *hub.Config) (idl.HubClient, error) {
			return mock_idl.NewMockHubClient(cli.ctrl), nil
		}
		err := cli2.InitClusterService("/tmp/invalid_file", false, false)
		if err == nil || !strings.Contains(err.Error(), testStr) {
			t.Fatalf("got %v, want %s", err, testStr)
	...
	t.Run("fails if ValidateInputConfigAndSetDefaults returns error", func(t *testing.T) {
		...
		cli2.ValidateInputConfigAndSetDefaults = func(request *idl.MakeClusterRequest, cliHandler *viper.Viper) error {
			return fmt.Errorf(testStr)
		}
		ConnectToHub = func(conf *hub.Config) (idl.HubClient, error) {
			return mock_idl.NewMockHubClient(cli.ctrl), nil
	...
	t.Run("returns error if RPC returns error", func(t *testing.T) {
		...
		ConnectToHub = func(conf *hub.Config) (idl.HubClient, error) {
			hubClient := mock_idl.NewMockHubClient(cli.ctrl)
	...
	t.Run("returns error if stream receiver returns error", func(t *testing.T) {
		...
		cli2.ParseStreamResponse = func(stream cli.StreamReceiver) error {
			return fmt.Errorf(testStr)
		}
		err := cli2.InitClusterService("/tmp/invalid_file", false, false)
		if err != nil {
...
func resetConfHostnames() {
	Conf.Hostnames = []string{"cdw", "sdw1", "sdw2"}
}
...
func TestValidateInputConfigAndSetDefaults(t *testing.T) {
	...
	t.Run("fails if coordinator segment is not provided in input config", func(t *testing.T) {
		...
		v := viper.New()
		err := cli2.ValidateInputConfigAndSetDefaults(request, v)
	...
	t.Run("fails if IsGpServicesEnabled returns error", func(t *testing.T) {
		...
		defer initializeRequest()
		Conf.Hostnames = []string{"cdw", "sdw1"}
		expectedError := "following hostnames [sdw2 sdw2] do not have gp services configured. Please configure the services"

		cli2.IsGpServicesEnabled = func(req *idl.MakeClusterRequest) error {
			return fmt.Errorf(expectedError)
		}

		err := cli2.ValidateInputConfigAndSetDefaults(request, cliHandler)
		if err == nil || !strings.Contains(err.Error(), expectedError) {
			t.Fatalf("got: %v, want: %v", err, expectedError)
		}
	})
	t.Run("succeeds with info if encoding is not provided", func(t *testing.T) {
		...
		defer initializeRequest()
		defer resetConfHostnames()
		cli2.CheckForDuplicatPortAndDataDirectory = func(primaries []*idl.Segment) error {
			return nil
		}
...
func TestCheckForDuplicatePortAndDataDirectoryFn(t *testing.T) {
	...
	t.Run("fails if duplicate data-directory entry is found for a host", func(t *testing.T) {
		...
		err := cli2.CheckForDuplicatPortAndDataDirectory(primaries)
		if err == nil || !strings.Contains(err.Error(), expectedError) {
			t.Fatalf("got %v, want %v", err, expectedError)
		}
...
func TestValidateSegmentFn(t *testing.T) {
	...
	t.Run("Returns error if hostname and address both are empty string", func(t *testing.T) {
		...
		err := cli2.ValidateSegment(&idl.Segment{
			HostName:      "",
			HostAddress:   "",
			Port:          5000,
...
func TestSetDefaultLocaleFn(t *testing.T) {
	...
	t.Run("returns error if GetSystemLocale fails", func(t *testing.T) {
		...
		cli2.GetSystemLocale = func() ([]byte, error) {
			return []byte(""), fmt.Errorf("failure")
		}
		err := cli2.SetDefaultLocale(locale)
		if err == nil || !strings.Contains(err.Error(), expectedError) {
			t.Fatalf("got %v, want %v", err, expectedError)
		}
	})
...
func TestIsGpServicesEnabledFn(t *testing.T) {
	...
	t.Run("fails if some of hosts do not have gp services configured", func(t *testing.T) {
		...
		err := cli2.IsGpServicesEnabled(&idl.MakeClusterRequest{GpArray: &gparray})
		if err == nil || !strings.Contains(err.Error(), expectedError) {
			t.Fatalf("got %v, want %v", err, expectedError)
		}
	})
