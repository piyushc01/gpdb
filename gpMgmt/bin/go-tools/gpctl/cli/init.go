...
import (
	...
	"context"
	"fmt"
	gpservicecli "github.com/greenplum-db/gpdb/gpservice/gpservice-cli"
...
var (
	...
	ParseStreamResponse                  = gpservicecli.ParseStreamResponseFn
	...
	AskUserYesNo                         = utils2.AskUserYesNo
)
var (
	cliForceFlag  bool
	cliCleanFlag  bool
...
func RunInitClusterCmd(cmd *cobra.Command, args []string) error {
	...
	// initial basic gpservice-cli validations
	if len(args) == 0 {
...
func InitClusterServiceFn(inputConfigFile string, force, verbose bool) error {
	_, err := utils2.System.Stat(inputConfigFile)
	if err != nil {
		return err
	}
	...
	if err != nil {
		return utils2.FormatGrpcError(err)
	}
	...
	if err != nil {
		...
		_, statErr := utils2.System.Stat(fileName)
...
func IsMultiHome(hostlist []string) (isMultiHome bool, NameAddress map[string][]string, AddressNameMap map[string]string, err error) {
	...
	if err != nil {
		...
		return false, nil, nil, utils2.LogAndReturnError(fmt.Errorf("failed names of the host against address: %v", err))
	}
	isMultiHome = false
...
func GetSystemLocaleFn() ([]byte, error) {
	cmd := utils2.System.ExecCommand("/usr/bin/locale")
...
func IsGpServicesEnabledFn(req *idl.MakeClusterRequest) error {
	cli.hostnames = []string{req.GpArray.Coordinator.HostName}
	for _, seg := range req.GetPrimarySegments() {
		cli.hostnames = append(cli.hostnames, seg.HostName)
	}

	// remove any duplicate entries
	slices.Sort(cli.hostnames)
	cli.hostnames = slices.Compact(cli.hostnames)

	diff := utils2.GetListDifference(cli.hostnames, Conf.Hostnames)
	if len(diff) != 0 {
