...
var (
	...
	ParseStreamResponse                  = gpservice_cli.ParseStreamResponseFn
...
func initCmd() *cobra.Command {
	initCmd := &cobra.Command{
		...
		PreRunE: gpservice_cli.InitializeCommand,
...
func RunInitClusterCmd(cmd *cobra.Command, args []string) error {
	...
	if cliCleanFlag {
		...
		return InitCleanFn(gpservice_cli.Verbose)
	}
	...
	err := InitClusterService(args[0], cliForceFlag, gpservice_cli.Verbose)
	if err != nil {
		return err
	}
...
func InitCleanFn(verbose bool) error {
	...
	client, err := gpservice_cli.ConnectToHub(gpservice_cli.Conf)
	if err != nil {
		return err
	}
...
func InitClusterServiceFn(inputConfigFile string, force, verbose bool) error {
	...
	HubClient, err = gpservice_cli.ConnectToHub(gpservice_cli.Conf)
	if err != nil {
		return err
	}
	...
	if err != nil {
		...
		fileName := filepath.Join(gpservice_cli.Conf.LogDir, constants.CleanFileName)
...
func IsGpServicesEnabledFn(req *idl.MakeClusterRequest) error {
	...
	diff := utils.GetListDifference(hostnames, gpservice_cli.Conf.Hostnames)
	if len(diff) != 0 {
